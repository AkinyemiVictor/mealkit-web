...
export async function POST(req) {
  ...
  const { data, error } = await admin.rpc("assign_role", { user_id, role_name: role });
  if (error) {
  ...
  await logAdminEvent({ route: "/api/admin/assign-role", actor: user.email, user_id, role, ok: true });
